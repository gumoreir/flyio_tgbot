...
LINK_FERRAMENTA = "https://t.me/TEU_LINK_DA_FERRAMENTA"     # troca
LINK_GRUPO_PRIVADO = "https://t.me/TEU_GRUPO_PRIVADO"       # troca
...
# user_data keys:
# interest: "BACBO" | "AMBOS" | "DEVOLUCAO"
# payout_method: "MBWAY" | "USDC"
# payout_value: phone or wallet
# awaiting_proof: bool
# support_mode: bool
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Ok! Saíste do suporte. Se precisares novamente escreve /help 🙂")


async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Único handler de texto para evitar conflitos:
    - Se estiver em support_mode -> encaminha para admin (modo ticket)
    - Senão, se estiver a pedir payout_value -> guarda
    ...
    if context.user_data.get("support_mode"):
        ...
        # guarda relação message_id (admin) -> user_id para resposta bidirecional
    ...
    if context.user_data.get("payout_method") and not context.user_data.get("payout_value"):
        if len(text) < 5:
...
async def admin_reply_to_support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Se o ADMIN responder (reply) a uma mensagem de suporte enviada pelo bot,
    o bot envia essa resposta para o utilizador correspondente.
    """
    ...
    if not user_id:
        return  # não é uma mensagem de suporte rastreada

    # Enviar a resposta do admin para o utilizador
    await context.bot.send_message(
        chat_id=user_id,
...
def main():
    ...
    # admin support replies (tem de vir antes do handler geral de texto)
    app.add_handler(MessageHandler(filters.TEXT & filters.User(ADMIN_ID), admin_reply_to_support))

    # comprovativos
    app.add_handler(MessageHandler(filters.PHOTO, receber_foto))
    app.add_handler(MessageHandler(filters.Document.ALL, receber_documento))

    # texto geral (1 handler único)
